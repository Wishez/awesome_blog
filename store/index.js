...
export const actions = {
  ...
  async getAdminInfo ({ commit }) {
    ...
    commit('options/SET_ADMIN_INFO', res || {})
  },
  ...
  async getOpt ({ commit }) {
    const res = await service.getOpt()
                      .catch(err => console.error(err))
    commit('options/SET_WEB_OPTION', res || {})
  },
  ...
  async getHotArt ({ commit }) {
    const res = await service.getArts({ hot: true })
                      .catch(err => console.error(err))
    commit('article/SET_HOT_ART', res || { pagination: {}, list: [] })
  },
  ...
  async getTag ({ commit }) {
    const res = await service.getTag({ page_size: 100 })
                      .catch(err => console.error(err))
    commit('tag/SET_TAG', res || { pagination: {}, list: [] })
  },
  ...
  async loadCommentsByPostId ({ commit, state }, data) {
    ...
    setTimeout(async () => {
      ...
      if (res && res.code === 1) {
        let list
        if (res.pagination.current_page === 1) list = res
        else list = [...state.comment.data.data, ...res]
        // if (Object.is(data.sort, -1)) res.data.reverse()
  ...
  async getHero ({ commit, state }, data = { current_page: 1 }) {
    ...
    if (res && res.code === 1) {
      let list
      if (res.pagination.current_page === 1) list = res.list
      else list = [...state.heros.data.list, ...res.list]
  ...
  async getBook ({ commit, state }, data = { current_page: 1, page_size: 6 }) {
    ...
    if (res && res.code === 1) {
      let list
      if (res.pagination.current_page === 1) list = res.list
      else list = [...state.book.data.list, ...res.list]
